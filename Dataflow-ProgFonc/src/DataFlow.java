...
public class DataFlow {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(ANSI_BLUE + "========== DÉMARRAGE DU DATAFLOW ==========" + ANSI_RESET);
        System.out.println(ANSI_BLUE + "Date/heure: " + startTime.format(formatter) + ANSI_RESET);
        System.out.println(ANSI_BLUE + "Thread principal: " + Thread.currentThread().getName() + ANSI_RESET);


        // Création du ForkJoinPool custom pour un meilleur contrôle du parallélisme
        ForkJoinPool customThreadPool = new ForkJoinPool(Runtime.getRuntime().availableProcessors());

        System.out.println(ANSI_BLUE +  "\n========== DÉPLOIEMENT DES CAPTEURS (SOURCE 1) ==========" + ANSI_RESET);
        ...
        System.out.println(ANSI_BLUE + "SOURCE 1: Nombre total de capteurs déployés: " + tousLesCapteurs.size() + ANSI_RESET);
        ...
        System.out.println(ANSI_BLUE + "\nRépartition des capteurs par localisation:" + ANSI_RESET);

        // POINT DE BIFURCATION 1: Le flux de capteurs se divise par localisation
        System.out.println(ANSI_BLUE + "\n*** POINT DE BIFURCATION 1: Répartition par localisation ***" + ANSI_RESET);

        CompletableFuture<List<Capteur>> capteursBatimentAFuture = CompletableFuture.supplyAsync(() -> {
            System.out.println(ANSI_BLUE +  "Thread " + Thread.currentThread().getName() + ": Traitement des capteurs du Bâtiment A" + ANSI_RESET);
        ...
        CompletableFuture<List<Capteur>> capteursBatimentBFuture = CompletableFuture.supplyAsync(() -> {
            System.out.println(ANSI_BLUE +  "Thread " + Thread.currentThread().getName() + ": Traitement des capteurs du Bâtiment B" + ANSI_RESET);
        ...
        CompletableFuture<List<Capteur>> capteursSerreFuture = CompletableFuture.supplyAsync(() -> {
            System.out.println(ANSI_BLUE + "Thread " + Thread.currentThread().getName() + ": Traitement des capteurs de la Serre" + ANSI_RESET);
        ...
        System.out.println(ANSI_BLUE +  "\nBâtiment A - Capteurs: " + capteursBatimentA.size() + ANSI_RESET);
        capteursBatimentAParType.forEach((type, capteurs) ->
                System.out.println("  - " + type + ": " + capteurs.size()));

        System.out.println(ANSI_BLUE +  "\nBâtiment B - Capteurs: " + capteursBatimentB.size() + ANSI_RESET);
        capteursBatimentBParType.forEach((type, capteurs) ->
                System.out.println("  - " + type + ": " + capteurs.size()));

        System.out.println(ANSI_BLUE +  "\nSerre Extérieure - Capteurs: " + capteursSerre.size()+ ANSI_RESET);
        capteursSerreParType.forEach((type, capteurs) ->
                System.out.println("  - " + type + ": " + capteurs.size()));

        System.out.println(ANSI_BLUE +  "\n========== COLLECTE DES DONNÉES (SOURCE 2) ==========" + ANSI_RESET);

        // SOURCE 2: Création du producteur de mesures et génération des mesures
        System.out.println(ANSI_BLUE + "Génération des mesures pour tous les capteurs..." + ANSI_RESET);
        ...
        System.out.println(ANSI_BLUE + "SOURCE 2: Nombre total de mesures collectées: " + toutesMesures.size() + ANSI_RESET);

        // Affichage de quelques exemples de mesures
        System.out.println(ANSI_BLUE + "\nExemples de mesures collectées:" + ANSI_RESET);
        ...
        System.out.println(ANSI_BLUE + "\n*** POINT DE BIFURCATION 2: Analyses parallèles des mesures ***" + ANSI_RESET);

        // SOURCE 3 (dérivée): Création de mesures supplémentaires pour simuler des pics anormaux
        System.out.println(ANSI_BLUE + "\n========== GÉNÉRATION DE DONNÉES SUPPLÉMENTAIRES (SOURCE 3) ==========" + ANSI_RESET);
        ...
        System.out.println(ANSI_BLUE +  "SOURCE 3: Nombre de mesures anormales générées: " + mesuresAnormales.size() + ANSI_RESET);

        // Fusion des sources de données
        System.out.println(ANSI_BLUE + "\n*** POINT DE FUSION: Combinaison des sources de mesures ***"+ ANSI_RESET);
        List<Mesure> toutesLesMesuresCombinees = new ArrayList<>(toutesMesures);
        toutesLesMesuresCombinees.addAll(mesuresAnormales);
        System.out.println(ANSI_BLUE +  "Nombre total de mesures après fusion: " + toutesLesMesuresCombinees.size()+ ANSI_RESET);

        System.out.println(ANSI_BLUE + "\n========== ANALYSE DES DONNÉES =========="+ ANSI_RESET);
        ...
        CompletableFuture<Void> analyseTemperatureBatAFuture = CompletableFuture.runAsync(() -> {
            try {
                customThreadPool.submit(() -> {
                    System.out.println(ANSI_BLUE + "\n--- Analyse de température - Bâtiment A ---"+ ANSI_RESET);
                    ...
                    System.out.println(ANSI_BLUE +  "Mesures collectées pour la température (Bâtiment A): " + mesuresTemperatureBatA.size() + ANSI_RESET);
                    ...
                    System.out.println(ANSI_BLUE +  "Température moyenne: " + String.format("%.1f", stats.getAverage()) + " °C" + ANSI_RESET);
                    System.out.println(ANSI_BLUE +  "Température minimale: " + String.format("%.1f", stats.getMin()) + " °C" + ANSI_RESET);
                    System.out.println(ANSI_BLUE +  "Température maximale: " + String.format("%.1f", stats.getMax()) + " °C" + ANSI_RESET);
                    ...
                    if (!temperaturesAnormales.isEmpty()) {
                        System.out.println(ANSI_RESET +  "Alertes de température détectées (" + temperaturesAnormales.size() + "):"  + ANSI_RESET);
            ...
            try {
                customThreadPool.submit(() -> {
                    ...
                    if (!co2Elevees.isEmpty()) {
                        System.out.println(ANSI_RED +  "Alertes de CO2 élevé détectées (" + co2Elevees.size() + "):" + ANSI_RESET);
            ...
            try {
                customThreadPool.submit(() -> {
                    ...
                    if (!humiditeBasse.isEmpty()) {
                        System.out.println(ANSI_RED +  "Alertes d'humidité basse détectées (" + humiditeBasse.size() + "):" + ANSI_RESET);
        ...
        System.out.println(ANSI_GREEN + "\n========== RAPPORT FINAL ==========" + ANSI_RESET);
        System.out.println(ANSI_GREEN + "Date/heure de fin: " + endTime.format(formatter) + ANSI_RESET);
        System.out.println(ANSI_GREEN + "Durée totale d'exécution: " + durationInSeconds + " secondes" + ANSI_RESET);
        System.out.println(ANSI_GREEN + "Nombre de capteurs traités: " + tousLesCapteurs.size() + ANSI_RESET);
        System.out.println(ANSI_GREEN + "Nombre de mesures analysées: " + toutesLesMesuresCombinees.size() + ANSI_RESET);

        System.out.println(ANSI_GREEN + "\nRésumé des alertes et actions:" + ANSI_RESET);
        System.out.println(ANSI_GREEN + "1. Bâtiment A: Suivi des températures anormales et ajustement climatisation" + ANSI_RESET);
        System.out.println(ANSI_GREEN + "2. Bâtiment B (Amphithéâtre): Surveillance du CO2 et activation ventilation si nécessaire" + ANSI_RESET);
        System.out.println(ANSI_GREEN + "3. Bâtiment B (Laboratoire): Maintien de la stabilité de pression" + ANSI_RESET);
        System.out.println(ANSI_GREEN + "4. Serre Extérieure: Gestion de l'humidité et arrosage automatique si nécessaire" + ANSI_RESET);

        customThreadPool.shutdown();

        System.out.println(ANSI_GREEN + "\n========== FIN DU DATAFLOW ==========" + ANSI_RESET);
